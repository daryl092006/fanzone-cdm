...
async function debugSpain() {
    const res = await fetch('https://worldcup26.ir/get/games');
    const data = await res.json();
    const spainGames = data.games.filter(g => g.home_team_name_en && (g.home_team_name_en.includes('Spain') || g.away_team_name_en.includes('Spain')));
    console.log("API Games with Spain:", spainGames);
    
    const { data: dbMatches } = await supabase.from('matches').select('*');
    const matchedDB = dbMatches.filter(m => m.team_home.includes('Spain') || m.team_away.includes('Spain') || m.team_home.includes('Espagne') || m.team_away.includes('Espagne'));
    
    console.log("DB Matches for Spain/Espagne:");
    matchedDB.forEach(m => {
        console.log(`  - DB Match: "${m.team_home}" vs "${m.team_away}"`);
        console.log(`    Normalized: "${normalizeTeamName(m.team_home)}" vs "${normalizeTeamName(m.team_away)}"`);
    });
}
debugSpain();
